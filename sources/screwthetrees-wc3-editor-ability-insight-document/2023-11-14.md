...
-   If you set values of "Damage Per Target" and "Maximum Number of
    ...
    *additional* projectiles will be launched. (Prometheus3375)
    ...
    -   If X = 0, then the ability will target all enemies in its range.
        > (Prometheus3375)

    -   If X = 1, then the ability will target 1 enemy in its range but
        > two projectiles appear due to the normal attack flying against
        > the primary target. (Prometheus3375)
...
-   The TownHall version only dictates the range from which to search
    ...
    -   Effectively it just causes all nearby peasants in range to cast
        > "Call to arms".
...
-   Changing the changed unit type of militia to peasant effectively
    ...
    -   The peasant cannot use Repair/Build/Gather while in militia form
        > regardless though.

    -   Other workers cannot use them either.

    -   If the other unit does not have the same Call To Arms ability as
        > the peasant, the game crashes when the duration runs out.

    -   "Normal Form Unit" Does not appear to actually matter, but keep
        > it proper anyway.
...
-   Setting the Cargo Capacity over 11 and having units entering causes
    ...
    -   Setting it below 0 and having units entering causes the game to
        > crash.
...
-   "Attack Speed Modifier" and "Movement Speed Modifier"
    ...
    -   At any value below 0.0 the speed decreases by that amount (i.e.
        > at -.5 the attacks should be twice as slow) includes the
        > animation.
...
-   Attacks Prevented turns off all kinds of the defined attack types,
    ...
    only have \"Tree\", \"Debris\" or \"Wall\" in the \"Combat - Attack
    x - Targets Allowed\" field; e.g. most workers have an invisible
    second attack that can only target trees (It's based on the
    AttackType of the unit). (Selaya)
...
-   "Data - Mana per Summoned Hitpoint" controls the mana spent by the
    ...
    -   The ability cannot be cast when the above formula returns a real
        > value that is greater than your current mana.
...
-   The two duration fields, "Hero" and "Normal" don't do anything.
    ...
    -   Upon cast, this ability resets the expiration timers of summoned
        > units, if they had one.

-   This ability is hard-coded to be only cast on enemy summons, the
    "Stats - Targets Allowed" field only concerns what kinds of summons
    these are, be it units, wards, or even buildings.
-   Just like Charm, this ability removes all debuffs, including Doom,
    on the target upon cast.

[]{#anchor-9}-Defend / -Magic Defense:
...
-   Flare count dictates how many times the skill can be used.
    ...
    -   Setting negative will instead do so that every time the skill is
        > used, it will keep decreasing (i.e. -1 =\> -2) and etc. every
        > time it's cast. Effectively making a "Times ability has been
        > cast" counter.
...
[]{#anchor-15}**-Flying Machine Bombs:**
...
[]{#anchor-16}**-Fragmentation Shards:**
...
-   When toggling on autocasting, the unit will attempt the unit with
    ...
    -   Stopping the unit or giving another order will cause the unit to
        > reset this behavior.

    -   If the priest is idle it will also toggle between targets
        > occasionally.

[]{#anchor-18}-Inner Fire:
...
-   Life Regen Rate.
    ...
    -   Causes the unit to receive a healing over time of the specified
        > amount per second. Does not modify unit life regeneration, for
        > example standard undead units will still receive this healing
        > over time when not on blight. Values less than 0 are not
        > damage but instead are negative healing over time. Does not
        > reward any kill credit for deaths it causes.

        -   Basically Life regen rate works like Unholy Aura, in that it
            only adds a percentage value to your health regeneration.
            (RoyGBV)

[]{#anchor-19}**-Invisibility:**
...
-   Targeting an enemy unit causes all of its positive buffs to fling as
    a projectile to your nearest ally (prioritizes allies who don\'t
...
-   Setting the health to a negative value causes the MK to die.
    ...
    -   Once the Avatar duration runs out his health gets turned back to
        > normal.
...
[]{#anchor-34}**-Bash / -Critical Strike:**
...
-   If a unit has both Bash and Critical Strike and they trigger
    ...
    -   Only the highest damage from Damage Multiplier value is actually
        > applied to the enemy unit.(Only the highest critical actually
        > counts).
...
-   Plays \"Caster art\" effect whenever evasion happens (You must set
    the chance to miss / chance to evade above 0.0) (KitsuneTailsPrower)
...
-   \"Value - Cast Time\" is the interval between waves.
    ...
    -   Setting this value to 0.01 or below triggers only 1 Wave and an
        > Unit can only be affect one time by Blizzard Waves using this
        > specific \"Blizzard (Caster)\" Buff until lost again, which
...
-   \"Value - Duration Hero/Unit\" Defines the duration of the DpS Buff
    on Hero/Unit
...
-   \"Value - Max Damage\" Defines the maximum Damage of one Wave
    ...
    -   Above 0 Damage will be split equally to all affected targets, if
        > a Wave would deal to much damage.

    -   Below 0, Changes it into an AoE Healing Spell, regardless of the
        > default damage.
...
-   "Amount Healed/Damaged\"
    ...
    -   Setting this to below 0 reverses the effect (Deal full damage to
        > allies and heal enemies for half damage).
...
[]{#anchor-46}**-Summon Water Elemental:**
...
-   "Attack/Movement Speed Reduction" defines how much those are
    ...
    -   Setting this to any value above 0 causes the unit to attack/move
        > slower.

    -   Setting this to any value below 0 causes the unit to attack/move
        > faster.

[]{#anchor-48}\@Orc:

[]{#anchor-49}**-Ancestral Spirit:**
...
-   Enabling enemy targeting in combination with the 'Devour - Cargo'
    ...
    -   Changing the targets allowed to include enemies on the standard
        > Cargo Hold ability does nothing
...
-   Damage Interval is the tickrate between damage.
    ...
    -   Setting this value below or at 0 causes it to devastatingly kill
        > anything it hits. by triggering several times every 0.01
        > seconds.
...
-   "Data - Melee attack range"
    ...
    -   Setting this allows for you to change the attack range of units.
        > (Tal0n)

    -   Only works with certain units, reasons why currently unknown.
        > (Tal0n)

[]{#anchor-59}-Envenomed Spears:
...
[]{#anchor-61}-Healing Ward / -Sentry Ward:
...
-   Salvage Cost Ratio seems to control the ratio of damage to resources
    ...
    -   Just like Unsummon Building, "Data - Salvage Cost Ratio" refers
        > to the total percentage of the building's gold and lumber cost
        > to be returned upon its destruction, and not per attack.
    ...
    -   "Data - Accumulation Step" has some differences from its
        ...
        -   For example, a unit' attack deals 100 Chaos Damage on a
            ...
            -   Sometimes though, due to decimal rounding calculations,
                > resources gained will be a point more or less than the
                > supposed percentage amount, it becomes more prevalent
                > when many units are attacking (eg: instead of 250G, it
                > becomes 249G).
    ...
    -   Adheres to Targets Allowed, allowing you to gain resources when
        > attacking enemy units. Does not work on allied/friendly units
        > and buildings.
...
[]{#anchor-69}**-Reinforced Burrows Upgrade:**
...
-   The damage sharing ignores distance and whenever they are actually
    ...
    -   Does not share the "unit takes damage" with enemies. But if you
        > throw the buff at an enemy they will get their own sided buff.
...
-   Data - Detonation Radius: Determines the AoE range of the Stun...
    But also the size of the AoE targeting image for the Casting Unit
    (Khyrberos, 1.26.0.6401)
...
[]{#anchor-79}-Chain Lightning / -Healing Wave:
...
-   Reveals higher ground while casting if standing near a cliff
    ...
    -   Provides flying vision around the caster for the duration of the
        > Split Time
...
-   "Data - Transition Time" (Khyrberos)
    ...
    -   Setting the TT to HIGHER than the duration: results in the hero
        > never truly becoming invisible. HOWEVER, he still sorta fades
        > out (kinda see it bad in the Portrait; shows through himself a
        > bit) unless you make set TT to the max (\~300). (Khyrberos)
...
-   Does not appear to conflict with current orders; You can make a
    Move/Attack order & then cast WW en route & he\'ll do it.
    (Khyrberos)
...
-   A ranged attack fired before using windwalk will not end the
    ...
    -   A slow moving projectile attack that is performed from inside a
        > previous windwalk won\'t end the current windwalk (but will
        > apply backstab damage from the previous windwalk.)
        > (Templarfreak)

[]{#anchor-92}\@Undead:

[]{#anchor-93}**-Absorb Mana:**
...
[]{#anchor-94}-Anti Magic Shell (Magic Resistance): //is this the "new"
one (says "(Magic Resistance)" in Editor Suffix), or the old/OG one?
\@TODO
...
-   "Data - Shield Life"
    ...
    -   A positive number causes the shield to only be able to withstand
        > magic until the shield runs out of health.

    -   A negative number causes it to break the first time it's hit.

        -   It still appears to deal damage.

[]{#anchor-95}-Aura of Blight:
...
[]{#anchor-96}-Blight Growth Large / Blight Growth Small:
...
[]{#anchor-97}-Blighted Gold Mine Ability:
...
-   "Data - Attack Speed Reduction", "Data - Damage Reduction" and
    "Data - Movement Speed Reduction"
...
-   (nchp1903) Tests were carried in 1.31 for the points above:
    ...
    -   "Triggers an ability being cast event every time.": Only happens
        > when the unit has Cargo Hold (Meat Wagon), or an ability based
        > on it, even if Cargo Capacity, Cast Range, Area of Effect are
        > set to 0, Targets Allowed is set to "None", and disabled by
        > trigger.
...
-   The only ability which applies it\'s effect in AoE when having
    splash attack. (DracoL1ch)
...
[]{#anchor-116}-Orb of Annihilation:
...
-   "Data - Accumulation Step" is the speed at which it gets destroyed.
    ...
    -   A value of zero causes the game to freeze. (you are dividing by
        > zero)
...
[]{#anchor-132}-Dark Ritual:

Tags: \[Mana Regen\] \[Mana Drain\] \[Healing\]

Notes: Would make for an excellent Dummy ability... if it didn't
slaughter the unit targeted.
...
-   "Data - Impact Delay"
    ...
    -   Higher values does not make the meteor come down slower. It only
        > makes it so that the spawning takes longer for the infernal.
...
-   "Data - Max Swarm Units Per Target"
    ...
    -   At negative values (or 0) they just won\'t target anyone and fly
        > around aimlessly.
...
-   "Data - Swarm Unit Type"
    ...
    -   Here's an interesting fact. Units that do not have the "Locust"
        > ability will be summoned normally and will function like a
        > normal unit.
...
-   Defense bonus changes flat bonus armor, works as intended with both
...
-   Returned Damage Factor changes how much damage is returned to the
    ...
    -    \* Above 1.0 the enemy takes more damage than you. This damage
        does not stack with Received Damage Factor damage.\
        \
        \
        **-Unholy Aura:**\
...
[]{#anchor-146}\@Night Elf:

[]{#anchor-147}-Abolish Magic:
...
-   Turns target invulnerable, removes all attached special effects from
    ...
    -   It also hides the unit if you use any other model in the Cyclone
        > and Cyclone (extra) other than the default Cyclone Model
        > (RoyGBV)
...
-   []{#anchor-151}Cycloned Units perform the stun order
...
-   "Data - Rip Delay"
    ...
    -   The amount of time it takes before the tree is removed from the
        > map.
...
-   "Data - Chance to Deflect"
    ...
    -   Deflects any ranged attack that is (Pierce, Magic or Spell), but
        > not missiles from Abilities (Barrage)
...
-   "Data - Damage Dealt"
    ...
    -   Affects the current attack when added by triggers in an unit is
        > attacked event.
...
-   "Data - Magic Damage Reduction"
    ...
    -   Values below 0 cause the unit to be healed instead when it would
        > receive Spell dmg. (emperor\_d3st)
...
[]{#anchor-158}-Entangled Gold Mine Ability:
...
-   "Data - Minimum damage"
    ...
    -   Damage instances smaller than the minimum damage may get bumped
        > up to the minimum. Needs more testing.

-   "Stats - Cooldown" and "Stats - Mana Cost" fields nonfunctional.
-   Plays Art - Caster effect when blocking damage (Somen00b)

[]{#anchor-162}-Lightning Attack:
...
[]{#anchor-169}-Replenish Mana and Life:
...
-   "Data - Number of Owls"
    ...
    -   Negative values causes it to count up by -1 every time it's cast

    -   0 causes unlimited uses.

-   Can only target trees; good for single-tree-targeting spells.

[]{#anchor-174}-Shadow Meld:
...
-   Upon casting, it issues a single attack order to 10 randomly chosen
    ...
    -   "Data - Prefer Hostiles", which houses the number of units to be
        > chosen, is hard-coded to be at 10.
...
-   If the range of this ability is higher than the caster's attack
    ...
    normal attack range... but the attack will always register as a
    miss. (EternalOne)

[]{#anchor-192}-Shadow Strike:
...
-   "Data - Decaying Power"
    ...
    -   -Speed and -Decaying Power -\> speed up speedUp increases over
        > time. (I encountered a bug where the units speed dropped to
        > its minimum after the spell ended)
...
-   If another buff or a custom buff is used it stacks with itself.
    ...
    -   If 2 tranquilities with different healing are cast at once, it's
        > gonna apply the effects of the first tranquility being cast.
...
-   "Data - Damage Bonus"
    ...
    -   Negative values causes all ranged units to lose damage instead.

-   Adheres to Targets Allowed

[]{#anchor-197}-Vengeance:
...
[]{#anchor-200}-Charge Gold and Lumber:
...
-   "Data - Gold/Lumber cost"
    ...
    -   If you set 1 value to negative and 1 to positive you can use it
        > to convert gold to lumber or vice versa, it will fail if you
        > lack the appropriate amount of the resource with the positive
        > value (KitsuneTailsPrower)
...
[]{#anchor-204}-Firebolt / -Frost Bolt:
...
[]{#anchor-208}-Life Regeneration Aura:
...
-   "Data - Percentage"
    ...
    -   When set to True it will heal a percentage of the unit's max HP
        > instead. A value of 0.01 is 1% healed per second.
        > (emperor\_d3st)
...
[]{#anchor-211}-Mana Regeneration: (emperor\_d3st)
...
-   Abilities can be marked as \"Hidden\" to prevent them showing up,
    but they will still be there and can be called by triggers.
...
-   Charmed units do use food values (except on summoned units), and can
    > exceed over your current food limits, meaning that you don't need
    > to "Build more Farms" to use Charm.
...
[]{#anchor-235}-Engineering Upgrade:
...
-   (nchp1903) From 1.31 onwards, Life Drain (but not Life Drain
    ...
    -   Only applicable to Black Arrows, Black Arrows (Neutral Hostile)
        > and abilities based on them. Doesn't work on Item Attack Black
        > Arrow Bonus or Item Black Arrow.
...
[]{#anchor-256}-Storm, Earth and Fire:
...
-   Channel ability - interrupting caster anyhow will cancel it\'s
    effect completely, ending it prematurely.
-   When the ability is used as an item ability and the summons are all
    ...
    -   The unit will actually die if there are no units nearby and its
        > death time has been reached.

    -   If there are no units nearby, the unit will play its death
        > animation.

    -   The unit can be referenced and affected by some triggers(mostly
        > visual)

[]{#anchor-257}-Summon Bear:
...
[]{#anchor-258}-Summon Hawk / -Summon Quillbeast:
...
-   "Data - Generation Count" (Notes below by RoyGBV)
    ...
    -   Effectively it's how many times they can split before they stop
        > splitting, minus one.
...
-   "Stats - Targets Allowed"
    ...
    -   Attacks need to actually land in order to count toward the split
        > count, meaning attacks that Miss do not count.

    -   Attacking friendly, allied, or your own units doesn't work, only
        > enemies.
...
[]{#anchor-264}\@Neutral Passive:

[]{#anchor-265}-Building Damage Aura:
...
-   "Data - Damage Per Second", "Data - Medium Damage Per Second",
    "Data - Small Damage Per Second"

    -   Negative values does not cause healing

-   Used by the Tornado unit from the "Tornado" ability. (RoyGBV)

[]{#anchor-266}-Slow Aura:
...
[]{#anchor-272}-AOE damage upon death:
...
[]{#anchor-273}-Blight Dispel Large / -Blight Dispel Small:
...
-   When loading in a unit with "Unit - Transported Size" value of more
    ...
    -   Cannot Load units with the Doom debuff. Units with the Parasite
        > debuff can be loaded in, but they will die inside the
...
-   "Stats - Area of Effect" is more like an unloading radius, which
    ...
    -   Setting this to low values causes the transport to be unable to
        > fully drop down units, as the target point would be filled
        > with units early on, capping out its AOE.

    -   Setting this to high values eliminates the problem altogether.

    -   Do note that Area of Effect is still taken into account when the
        > transport dies on pathable ground, and that it will instantly
        > kill the other units in the transport if the AOE is full.
...
-   One can give one hero multiple inventory skills to carry and use
    ...
    -   When the Hero gets revived, paused, reincarnated, \... Then only
        > the main inventory takes all the items again. To counter that
        > one needs to re-add the inventory skills. One of the inventory
        > skills can not be removed, but still can be added allowing the
        > hero to carry more and more items, when done.
...
[]{#anchor-280}-Load (Goblin Zeppelin/Ships):
...
-   "Stats - Cast Range "(Kino)
    ...
    -   If the transporting unit is mobile, it will attempt to load the
        > target from the 'Cast Range' value. If this value exceeds
...
[]{#anchor-282}-Permanent Immolation:
...
-   Copy of Immolation
    ...
    -   Having multiple levels does not work. Regular Immolation doesn't
        > switch to higher level values until it's deactivated and
        > reactivated, and since Permanent Immolation can't be
        > deactivated and reactivated, it always uses its level 1
        > values. (EternalOne)

-   Since 1.35+, it's possible to change an ability\'s real values,
    which makes it possible to make a multi-leveled Permanent
    Immolation, even though it will always use level 1's (0's in
    triggers) values. (PauloHPBender)

    -   

[]{#anchor-283}-Permanent Invisibility:
...
[]{#anchor-284}-Item Sight Range Bonus:
...
-   Stats - Levels
    ...
    -   Having multiple levels does not work - changing levels does not
        > change the Sight Range Bonus value.

[]{#anchor-285}-Phoenix Fire(flyer):
...
[]{#anchor-288}-Return Gold / Return Lumber:
...
[]{#anchor-291}-Select Hero / Select Unit:
...
[]{#anchor-292}-Sell Items / Sell Units:
...
[]{#anchor-298}-Waygate / -Waygate Ability:
...
[]{#anchor-304}-Clarity Potion / -Generic Item-Rejuv Effect / -Healing
Salve:
...
[]{#anchor-307}-Glyph of Fortification:
...
[]{#anchor-308}-Item Agility Gain / -Item Intelligence Gain / -Item
Strength Gain:
...
[]{#anchor-309}-Item Area Heal / -Mana Regain:
...
[]{#anchor-310}-Item Area tree / -wall damage:
...
[]{#anchor-312}-Item Attack Black Arrow Bonus:
...
[]{#anchor-313}-Item Attack Corruption Bonus:
...
[]{#anchor-314}-Item Attack Fire Bonus:
...
[]{#anchor-315}-Item Attack Frost Bonus:
...
[]{#anchor-316}-Item Attack Lightning Bonus:
...
[]{#anchor-317}-Item Attack Poison Bonus:
...
[]{#anchor-318}-Item Freeze Damage Bonus:
...
[]{#anchor-319}-Item Attack Speed Bonus:
...
[]{#anchor-320}-Item Capture The Flag:
...
[]{#anchor-325}-Item Doom Guard Summon / -Item Felhound Summon / -Item
Furbolg Summon / -Item Ice Revenant Summon / -Item Red Drake Summon /

[]{#anchor-326}-Item Rock Golem Summon / -Item Skeleton Summon / -Item
Ursa Warrior Summon / -Summon Headhunter:
...
[]{#anchor-327}-Item Experience Gain:
...
[]{#anchor-329}-Item Hero Stat Bonus:
...
-   Setting to 0 still causes the hero to gain a level or lose all
    XP\... (Needs more testing!)

[]{#anchor-333}-Item Life Bonus:
...
[]{#anchor-334}-Item Life Regeneration:
...
[]{#anchor-338}-Item Mana Regeneration:
...
[]{#anchor-339}-Item Move Speed Bonus:
...
[]{#anchor-340}-Item Permanent Damage Gain:
...
[]{#anchor-341}-Item Permanent Life Gain:
...
-   Setting to a negative value causes the hero to lose health. After
    ...
    -   At first it will have over 4000000000 health (int max). And will
        > keep decreasing.

        -   Health does not work as intended at this high level, causing
            invulnerability.

    -   Increasing the health back up past the threshold makes it return
        > to normal health. (StormKnight)

-   Setting it to 0 will cause nothing to happen. Good for dummy item
    pickup ability.

[]{#anchor-342}-Item Place Goblin Land Mine:
...
[]{#anchor-344}-Item Reveal Entire Map:
...
[]{#anchor-345}-Item Sight Range Bonus:
...
[]{#anchor-346}-Item Soul Possession / -Item Soul Theft:
...
-   Adheres to Targets Allowed, which causes some interesting things to
    ...
    -   Casting this on Buildings causes them to disappear like you
        ...
        -   When the carrying unit dies, the Buildings will appear
            ...
            -   When capturing an Undead building, the blight will stay
                > on where it originally was, but it can be dispelled
                > and cleared out by making non-Undead buildings there.
        ...
        -   The following will happen when Soul Theft is cast on a
            ...
            -   Undead: The construction progress continues while being
                > captured. When completed, Blight is not created until
                > it is freed, and when freed, the blight will be
            ...
            -   Buildings going through upgrades (Town Halls) will still
                > continue upgrading while captured, however it cannot
                > attack, cast abilities, or train units while captured.
    ...
    -   Casting this on your Town Hall causes workers who are gathering
        > resources to act like it\'s there and return resources on an
        > invisible spot.
    ...
    -   When cast on a Gold Mine, causes Human and Orc workers to simply
        > continue mining from an invisible source. When the carrying
        > unit dies, the AI of Peasants and Orcs will update, and they
        > will now start walking to the new location to harvest gold.
        ...
        -   For Undead, casting this on a Haunted Gold Mine causes it to
            ...
            unit's death location, and Acolytes will stop playing their
            animation and the white pentagonal effect on the ground
            disappears. However you can select and order your Acolytes
            to harvest gold from the new location normally again.

    -   Casting this on enemy Heroes causes them to disappear, but their
        > interface icons are still present and you will be revealed to
        > their team, almost as if you cast Devour on that Hero. Same
        > thing happens to non-Hero units.
...
[]{#anchor-347}-Item Temporary Area Armor Bonus:
...
[]{#anchor-348}-Item Temporary Invisibility:
...
[]{#anchor-349}-Item Temporary Invulnerability:
...
[]{#anchor-350}-Item Temporary Speed Bonus:
...
[]{#anchor-353}-Melee Cold Damage Bonus / -Melee Fire Damage Bonus /
...
[]{#anchor-359}-Powerup Area Healing:
...
-   Changing the orderid to a build ability transforms the spellbook to
    ...
    -   Shows the buildings that unit can already build in its building
        > list
...
[]{#anchor-365}-Spell Damage Reduction:
...
[]{#anchor-368}-Staff of Preservation:
...
[]{#anchor-370}-Staff of Teleportation:
...
-   Used by the item Wand of Mana Stealing.
    ...
    -   Seems to function like Dark Ritual, but seems to not always work
        > as intended.

[]{#anchor-372}-Tome of Retraining:
...
[]{#anchor-374}-Wand Of Shadowsight:
...
[]{#anchor-376}-Chaos / -Berserker Upgrade:
...
-   Normal abilities will be overwritten by the normal abilities on the
    new hero.
-   If any item or aura buff obtained (i.e. providing bonus armor),
    it\'s effect may become part of the unit forever, which heavily
    limits usage of this ability in common. Interaction depends on the
    base ability which is the source of the bonus.
...
[]{#anchor-377}-UnselectAble (Internal name: CAbilityLeash) :

Tags: \[Unique\]

Notes: Need to be defined in units/abilitydata.slk

[]{#anchor-378}RawCode: Alea
...
[]{#anchor-380}-Ability name:

Tags: Tags

Notes: Text

[]{#anchor-381}RawCode: xxxx
